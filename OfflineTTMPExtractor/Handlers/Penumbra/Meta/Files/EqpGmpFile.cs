...
public unsafe class ExpandedEqpGmpBase : MetaBaseFile {
  ...
  protected static ulong GetDefaultInternal(Penumbra.Interop.CharacterUtility.InternalIndex fileIndex, int setIdx, ulong def) {
    //var data = ( byte* )App.CharacterUtility.DefaultResource(fileIndex).Address;
    if (setIdx == 0) {
      setIdx = 1;
    }
    ...
    var control  = (ulong)0;//*( ulong* )data;
...
public sealed class ExpandedEqpFile : ExpandedEqpGmpBase, IEnumerable<EqpEntry> {
  public static readonly Penumbra.Interop.CharacterUtility.InternalIndex InternalIndex =
        Penumbra.Interop.CharacterUtility.ReverseIndices[ (int) CharacterUtility.Index.Eqp ];
...
public sealed class ExpandedGmpFile : ExpandedEqpGmpBase, IEnumerable<GmpEntry> {
  public static readonly Penumbra.Interop.CharacterUtility.InternalIndex InternalIndex =
        Penumbra.Interop.CharacterUtility.ReverseIndices[( int )CharacterUtility.Index.Gmp];
