...
public partial class Mod {
  internal static partial class Creator {
    ...
    public static void CreateMeta(DirectoryInfo directory, string? name, string? author, string? description, string? version,
      ...
      mod.Name = string.IsNullOrEmpty(name) ? mod.Name : new LowerString(name!);
      ...
      foreach (var groupFile in files) {
        ...
        try {
          if (reorder) {
            ...
            Logger.Log.Debug($"Moving {groupFile.Name} to {Path.GetFileName(newName)} due to reordering after multi group split.");
        ...
        try {
          ...
          Logger.Log.Information($"Splitting multi group {name} in {mod.Name} due to {options.Count} being too many options.");
          ...
          var match       = new Regex(@", Part (\d+)$").Match( name );
          ...
          Logger.Log.Debug($"Writing the first {IModGroup.MaxMultiOptions} options to {Path.GetFileName(oldPath)} after split.");
          ...
          Logger.Log.Debug($"Writing the remaining {options.Count - IModGroup.MaxMultiOptions} options to {Path.GetFileName(newPath)} after split.");
