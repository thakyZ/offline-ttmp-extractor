    ...
    try {
      ...
      switch (json[nameof(Type)]?.ToObject<GroupType>() ?? GroupType.Single) {
        case GroupType.Multi:
          return MultiModGroup.Load(mod, json, groupIdx);
        case GroupType.Single:
          return SingleModGroup.Load(mod, json, groupIdx);
      }
    } catch (Exception e) {
      Logger.Log.Error($"Could not read mod group from {file.FullName}:\n{e}");
    }
  ...
  private void SaveAllGroups() {
    foreach (var file in GroupFiles) {
      try {
        ...
        Logger.Log.Error($"Could not delete outdated group file {file}:\n{e}");
      }
    }
