...
public sealed partial class Mod {
  ...
  private ModDataChangeType LoadMeta() {
    ...
    if (!File.Exists(metaFile.FullName)) {
      Logger.Log.Debug($"No mod meta found for {ModPath.Name}.");
    ...
    try {
      ...
      Logger.Log.Error($"Could not load mod meta:\n{e}");
      return ModDataChangeType.Deletion;
    }
  }
  ...
  private void SaveMetaFile() {
    ...
    try {
      ...
      Logger.Log.Error($"Could not write meta file for mod {Name} to {metaFile.FullName}:\n{e}");
    }
  }
