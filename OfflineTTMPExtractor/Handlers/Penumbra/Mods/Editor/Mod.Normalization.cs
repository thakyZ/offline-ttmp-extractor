...
public partial class Mod {
  ...
  private struct ModNormalizer {
    ...
    public static void Normalize(Manager manager, Mod mod) {
      ...
      try {
        Logger.Log.Debug($"[Normalization] Starting Normalization of {mod.ModPath.Name}...");
        if (!normalizer.CheckDirectories()) {
          return;
        }

        Logger.Log.Debug("[Normalization] Copying files to temporary directory structure...");
        if (!normalizer.CopyNewFiles()) {
          return;
        }

        Logger.Log.Debug("[Normalization] Moving old files out of the way...");
        if (!normalizer.MoveOldFiles()) {
          return;
        }

        Logger.Log.Debug("[Normalization] Moving new directory structure in place...");
        if (!normalizer.MoveNewFiles()) {
          return;
        }

        Logger.Log.Debug("[Normalization] Applying new redirections...");
        normalizer.ApplyRedirections(manager);
      } catch (Exception e) {
        Logger.Log.Error($"Could not normalize mod:\n{e}"); //, "Failure", NotificationType.Error);
      } finally {
        Logger.Log.Debug("[Normalization] Cleaning up remaining directories...");
        normalizer.Cleanup();
      }
    }
    ...
    private bool MoveOldFiles() {
      try {
        ...
        Logger.Log.Error($"Could not move old files out of the way while normalizing mod mod:\n{e}"); //, "Failure", NotificationType.Error);
      }
    ...
    private bool MoveNewFiles() {
      try {
        ...
        Logger.Log.Error($"Could not move new files into the mod while normalizing mod mod:\n{e}"); //, "Failure", NotificationType.Error);
        foreach (var dir in _mod.ModPath.EnumerateDirectories()) {
          if (dir.FullName.Equals(_oldDirName, StringComparison.OrdinalIgnoreCase)
          || dir.FullName.Equals(_normalizationDirName, StringComparison.OrdinalIgnoreCase)) {
            continue;
